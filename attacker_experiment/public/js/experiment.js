...
const practice_parameters = [
    { period: 4, duty: 0.1, angle: 45, text: 'PRACTICE', font_size: 16, stripe_color: 0 },
    { period: 6, duty: 0.2, angle: 60, text: 'EXAMPLE' , font_size: 16, stripe_color: 200 },
];
...
const no_stripe_instruction = {
    ...
    choices: ['はじめる'], 
    post_trial_gap: 1000,
    data: { task: 'no_stripe_instruction' }
};
...
const no_stripe_parameters = [
    { period: 4, duty: 0, angle: 0, text: "PENCIL", font_size: 16, stripe_color: 200 },
    { period: 4, duty: 0, angle: 0, text: 'UNLOCK', font_size: 16, stripe_color: 200 },
    { period: 4, duty: 0, angle: 0, text: 'CHERRY', font_size: 16, stripe_color: 200 },
];
...
const no_stripe_trials = {
    ...
    on_timeline_start: function() {
        fetch('/set_trial', {
            ...
            body: JSON.stringify({
                current_trial: 0,
                test_parameters: no_stripe_parameters
            })
        });
    }
}
...
const stripe_colors = [128,200]; // 複数のstripe_colorを定義
const periods = [3,4];
const font_size = 16;
const texts = [
...
let test_parameters = param_pairs.map((pair, idx) => ({
    ...
    duty: pair.duty,
    angle: pair.angle,
    stripe_color: pair.stripe_color, // stripe_colorを追加
