...
// 画像表示用のHTMLを取得
function getStripeImageHTML({canvas_id}) {
    return `<img id="${canvas_id}" style="display:block;margin:auto;width:100%;height:auto;" />`;
}

// 画像を読み込む
function loadStripeImage(params, canvas_id) {
    ...
    .catch(error => console.error('画像生成エラー:', error));
}
...
const practice_parameters = [
    { period: 4, duty: 0.3, angle: 45, text: 'PRACTICE', font_size: 16, stripe_color: 128, blend_alpha: 1, blur_radius: 2 },
    { period: 3, duty: 0.1, angle: 135, text: 'EXAMPLE', font_size: 16, stripe_color: 0, blend_alpha: 1, blur_radius: 3 },
];
...
const practice_trials = {
    timeline: practice_parameters.map((params, idx) => {
        const canvas_id = `practice-image-${idx}`;
        return {
            type: jsPsychSurveyText,
            preamble: getStripeImageHTML({canvas_id}),
            on_load: function() {
                loadStripeImage(params, canvas_id);
            },
            questions: [
...
const no_stripe_parameters = [
    { period: 4, duty: 0, angle: 0, text: "TABLES", font_size: 16, stripe_color: 200, blend_alpha: 0.8, blur_radius: 2 },
    { period: 4, duty: 0, angle: 0, text: 'LITTLE', font_size: 16, stripe_color: 200, blend_alpha: 0.8, blur_radius: 2 },
    { period: 4, duty: 0, angle: 0, text: 'FISHES', font_size: 16, stripe_color: 200, blend_alpha: 0.8, blur_radius: 2 },
];
...
const duties = [0.5];
const stripe_colors = [255]; 
const periods = [2,3,4];
const font_size = 16;
const blend_alphas = [1];
const blur_radiuses = [1, 2, 3, 4]; 
const texts = [
    ...
    duties.forEach(duty => {
        stripe_colors.forEach(stripe_color => {
            periods.forEach(period => {
                blend_alphas.forEach(blend_alpha => {
                    blur_radiuses.forEach(blur_radius => {
                        param_pairs.push({ angle, duty, stripe_color , period, blend_alpha, blur_radius });
                    });
                });
            });
        });
...
let test_parameters = param_pairs.map((pair, idx) => ({
    ...
    font_size: font_size,
    blend_alpha: pair.blend_alpha,
    blur_radius: pair.blur_radius,
}));
...
const trials = {
    timeline: test_parameters.map((params, idx) => {
        const canvas_id = `main-image-${idx}`;
        return {
            type: jsPsychSurveyText,
            preamble: getStripeImageHTML({canvas_id}),
            on_load: function() {
